...
class MUSICMix21Dataset(BaseDataset):
    def __init__(self, list_sample, cur_num_mix, opt, **kwargs):
        super(MUSICMix21Dataset, self).__init__(
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            csv_lis_path = "YOURPATH/Music_21/testmusic21.csv"
            csv_lis = []
        ...
        #use for matching in transformer
        instrument_dict = {'accordion':0, 'acoustic_guitar':1, 'cello':2, 'clarinet':3, 'erhu':4, 'flute':5, 'saxophone':6, 'trumpet':7, 'tuba':8, 
        'violin':9, 'xylophone':10, 'bagpipe':11, 'banjo':12, 'bassoon':13, 'congas':14, 'drum':15, 'electric_bass':16, 'guzheng':17, 'piano':18,
        'pipa':19,'ukulele':20}
        ...
        for n, infoN in enumerate(infos):
            ...
            for i in range(self.num_frames):
                ...
                path_frames[n].append(
                    os.path.join("YOURPATH/MUSIC_21/frames",
                        path_frameN[1:],
                        '{:06d}.jpg'.format(center_frameN + idx_offset)))
                path_frames_ids[n].append(center_frameN + idx_offset)
            path_frames_det[n] = os.path.join("YOURPATH/MUSIC_21/detection_results",
                        path_frameN[1:]+'.npy')

            #include motions
            path_motions[n] = os.path.join("YOURPATH/MUSIC_21/motion_features",
                        path_frameN[1:]+'.npy')
            path_audios[n] = os.path.join("YOURPATH/MUSIC_21/audio", path_audioN[1:])
        ...
        try:
            for n, infoN in enumerate(infos):
                frames[n] = self._load_frames_det_21(path_frames[n], path_frames_ids[n], path_frames_det[n],class_list[n])      
                # jitter audio
