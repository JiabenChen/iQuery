...
from dataset import MUSICMix21Dataset
...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args, cur_num_mix=2):
        ...
        for n in range(N):
            pred_masks[n] = self.net_maskformer(feat_latent, feat_sound, classes[n], feat_frames[n], motions[n], motion_masks[n])
...
def output_visuals(vis_rows, batch_data, outputs, args, cur_num_mix):
    ...
    # convert into numpy
    mag_mix = mag_mix.numpy()
    mag_mix_vis = mag_mix_.detach().cpu().numpy()
    ...
    for j in range(B):
        ...
        tmp1 = mix_amp[::-1, :, :]
        tmp1 = cv2.cvtColor(tmp1, cv2.COLOR_RGB2BGR)
        cv2.imwrite(os.path.join(args.vis, filename_mixmag), tmp1)
        ...
        for n in range(N):
            ...
            tmp2 = gt_mag[::-1, :, :]
            tmp2 = cv2.cvtColor(tmp2, cv2.COLOR_RGB2BGR)
            tmp3 = pred_mag[::-1, :, :]
            tmp3 = cv2.cvtColor(tmp3, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(args.vis, filename_gtmag), tmp2)
            cv2.imwrite(os.path.join(args.vis, filename_predmag), tmp3)
...
def main(args):
    ...
    # Dataset and Loader
    dataset_train = MUSICMix21Dataset(
        args.list_train, args.num_mix, args, split='train')
    dataset_val = MUSICMix21Dataset(
    ...
    if args.mode == 'eval':
        args.testing = True
        evaluate(netWrapper, loader_val, history, 0, args, args.num_mix)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        args.id += '-{}encoder'.format(args.MASK_FORMER_ENC_LAYERS)
